...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		#if LUA_ALLOWED
		var foldersToCheck:Array<String> = Mods.directoriesWithFile(Paths.getSharedPath(), 'songs/' + songName + '/');
		for (folder in foldersToCheck)
			for (file in FileSystem.readDirectory(folder))
			{
				if(file.toLowerCase().endsWith('.lua'))
	...
	private function cachePopUpScore()
	{
		...
		for (i in 0...10)
			Paths.image(uiPrefix + 'ui/combo/num' + i + uiSuffix);
	}
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		rating.loadGraphic(Paths.image('ui/ratings/' + uiPrefix + daRating.image + uiSuffix));
		rating.screenCenter();
		...
		var comboSpr:FlxSprite = new FlxSprite().loadGraphic(Paths.image(uiPrefix + 'ui/combo/combo' + uiSuffix));
